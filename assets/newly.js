  ...
  try {
    ...
    grid.innerHTML = `
      <div class="col-span-3 flex justify-center items-center py-10 flex-col">
        ...
        <p class="text-[color:var(--brand-magenta)] font-medium">Loading cakes...</p>
      </div>
    `;
    ...
    newCakes.forEach(item => {
      ...
      card.innerHTML = `
        <span class="absolute top-4 left-4 bg-[color:var(--brand-magenta)] text-white text-xs font-semibold px-3 py-1 rounded-full shadow-md">NEW</span>
        <div class="overflow-hidden rounded-2xl mb-5">
          <img src="${imagePath}" alt="${item.name}"
            class="rounded-2xl w-full h-60 object-cover transform group-hover:scale-105 transition-transform duration-500" />
        </div>
